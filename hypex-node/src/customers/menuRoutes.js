...
/* ── مزامنة / ربط عملاء Oracle (واجهة Node + أدوات OCI عبر PHP) ── */
function runOracleCli(action) {
  ...
  const candidates = [
    ...
    'C:\\xampp\\php\\php.exe',
    'C:\\xampp\\php\\php.exe',
    path.join('C:', 'xampp', 'php', 'php.exe'),
    'php',
  ].filter(Boolean);
  let lastErr = 'تعذّر تشغيل PHP';
  for (const phpBin of candidates) {
    const r = spawnSync(phpBin, [script, '--action=' + action], {
      encoding: 'utf8',
      timeout: 120000,
      windowsHide: true,
    });
    if (r.error && r.error.code === 'ENOENT') {
      lastErr = 'PHP غير موجود في المسار: ' + phpBin;
      continue;
    }
    ...
    try {
      ...
      lastErr = raw || String(r.stderr || r.error || 'تعذّر قراءة نتيجة المزامنة');
    }
  }
...
router.get('/customers/oracle-sync', guard('oracle_customers_sync'), async (req, res) => {
  ...
  let status = { ok: true, message: '', mapping: null, enabled: false, has_driver: false };
  try {
    status = runOracleCli('status');
  } catch (e) {
    status = { ok: false, message: e.message || String(e) };
  }
  const msg = String(req.query.msg || '');
  const err = String(req.query.err || '');
  const mapLabel = status.mapping
    ? `${status.mapping.owner}.${status.mapping.table}`
    : '— غير محفوظ —';
  const lastSync = status.mapping && status.mapping.last_synced_at ? status.mapping.last_synced_at : '—';
  const setupUrl = ui.phpUrl('oracle_customers_sync', '&force_php=1');
  ...
  const body = `
    <div class="si-stage">
      ${ui.hero({
        ...
        subtitle: 'إعداد الربط مع جداول Oracle ومزامنة العملاء إلى Hypex.',
        actions: [
          { label: 'لوحة العملاء', href: HUB },
      ...
      ${
        msg
          ? `<div class="si-surface" style="padding:.85rem 1rem;margin-bottom:.75rem;border-color:#86efac;background:#f0fdf4"><strong>${ui.esc(
              msg
            )}</strong></div>`
          : ''
      }
      ${
        err
          ? `<div class="si-surface" style="padding:.85rem 1rem;margin-bottom:.75rem;border-color:#fca5a5;background:#fef2f2"><strong>${ui.esc(
              err
            )}</strong></div>`
          : ''
      }
      <section class="si-surface" style="margin-bottom:.75rem">
        ...
        <div style="padding:1rem 1.1rem;display:grid;gap:.55rem;grid-template-columns:repeat(auto-fit,minmax(12rem,1fr))">
          ...
          <div><div class="muted" style="font-size:.75rem">الإعداد / المشغّل</div><strong>${
            status.enabled ? 'مفعّل' : 'غير مفعّل'
          } · ${status.has_driver ? 'OCI جاهز' : 'OCI غير محمّل'}</strong></div>
        </div>
        <div style="padding:0 1.1rem 1rem;display:flex;flex-wrap:wrap;gap:.5rem">
          <form method="post" action="/customers/oracle-sync/test">
            <button class="si-btn" type="submit">اختبار الاتصال</button>
          </form>
          <form method="post" action="/customers/oracle-sync/run">
            <button class="si-btn si-btn--primary" type="submit">مزامنة بالتعيين المحفوظ</button>
          </form>
          <a class="si-btn" href="${ui.esc(setupUrl)}" target="_blank" rel="noopener">فتح إعداد الربط (جداول/أعمدة)</a>
        </div>
        <p class="muted" style="margin:0 1.1rem 1rem;font-size:.82rem;line-height:1.45">
          إعداد الربط الأول (اختيار جدول Oracle وربط الأعمدة) يحتاج مشغّل Oracle على PHP.
          بعد حفظ التعيين يمكن المزامنة من الأزرار أعلاه دون الرجوع للقائمة العامة.
        </p>
      </section>
      <section class="si-surface">
        <div class="si-surface-head">
          <h2>شاشة إعداد الربط والمزامنة</h2>
          <a class="si-btn" href="${ui.esc(setupUrl)}" target="_blank" rel="noopener">تكبير</a>
        </div>
        <iframe class="php-embed-frame php-embed-frame--in-card" src="${ui.esc(
          setupUrl
        )}" title="إعداد مزامنة Oracle" style="min-height:70vh;width:100%;border:0;background:#fff"></iframe>
      </section>
    </div>`;
...
router.post('/customers/oracle-sync/test', guard('oracle_customers_sync'), (req, res) => {
  const r = runOracleCli('test');
  const key = r.ok ? 'msg' : 'err';
  res.redirect(
    '/customers/oracle-sync?' + key + '=' + encodeURIComponent(r.message || (r.ok ? 'نجح الاتصال' : 'فشل الاتصال'))
  );
});

router.post('/customers/oracle-sync/run', guard('oracle_customers_sync'), (req, res) => {
  const r = runOracleCli('sync');
  const key = r.ok ? 'msg' : 'err';
  res.redirect(
    '/customers/oracle-sync?' + key + '=' + encodeURIComponent(r.message || (r.ok ? 'تمت المزامنة' : 'فشلت المزامنة'))
  );
});
