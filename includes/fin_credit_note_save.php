...
function handle_fin_credit_note_save(): void
{
    ...
    if (!verify_csrf($_POST['_csrf'] ?? null)) {
        ...
        redirect(app_url('index.php?r=credit_notes'));
    }

    $pdo = db();
    if (!fin_credit_note_ensure_schema($pdo)) {
        $msg = 'جداول إشعارات الدائنة غير موجودة. نفّذ ترحيل قاعدة البيانات (schema).';
        if ($wantsJson) {
    ...
    try {
        ...
        $savedId = fin_credit_note_save($pdo, [
        ...
        $note = fin_credit_note_fetch($pdo, $savedId);
        if ($wantsJson) {
            json_invoice_save_response(true, [
                'id' => $savedId,
                'note_no' => (string) ($note['note_no'] ?? ''),
                'message' => 'تم حفظ إشعار الدائنة.',
            ]);
        }
        flash_set('success', 'تم حفظ إشعار الدائنة.');
        redirect(app_url('index.php?r=credit_notes&id=' . $savedId));
        ...
        redirect(app_url('index.php?r=credit_notes' . ($rid > 0 ? '&id=' . $rid : '')));
    } catch (Throwable $e) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
        }
