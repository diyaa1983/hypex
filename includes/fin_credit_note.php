...
function fin_credit_note_probe_table(PDO $pdo): bool
{
    try {
        $pdo->query('SELECT id FROM fin_credit_note LIMIT 1');
...
function fin_credit_note_has_table(PDO $pdo): bool
{
    static $ok = false;
    if ($ok) {
        return true;
    }
    if (fin_credit_note_probe_table($pdo)) {
        $ok = true;
    }
...
function fin_credit_note_ensure_schema(PDO $pdo): bool
{
    if (fin_credit_note_has_table($pdo)) {
        return true;
    }
    require_once app_path('includes/sql_migration.php');
    sql_migration_run_file($pdo, 'database/migrations/030_fin_debit_credit_notes.sql');

    return fin_credit_note_probe_table($pdo);
}

function fin_credit_note_next_no(PDO $pdo): string
{
    $st = $pdo->query(
        "SELECT note_no FROM fin_credit_note WHERE note_no REGEXP '^CN-[0-9]+$' ORDER BY id DESC LIMIT 1"
    );
    $last = $st ? $st->fetchColumn() : false;
    $n = 0;
    if (is_string($last) && preg_match('/^CN-(\d+)$/', $last, $m)) {
        $n = (int) $m[1];
    }
...
function fin_credit_note_fetch(PDO $pdo, int $id): ?array
{
    if ($id < 1) {
        return null;
    }
    $st = $pdo->prepare('SELECT * FROM fin_credit_note WHERE id = ? LIMIT 1');
    $st->execute([$id]);
...
function fin_credit_note_nav_neighbor_id(PDO $pdo, int $id, string $dir): ?int
{
    if ($id < 1) {
        return null;
    }
    if ($dir === 'prev') {
        $st = $pdo->prepare('SELECT id FROM fin_credit_note WHERE id < ? ORDER BY id DESC LIMIT 1');
    } else {
        $st = $pdo->prepare('SELECT id FROM fin_credit_note WHERE id > ? ORDER BY id ASC LIMIT 1');
    }
    $st->execute([$id]);
    $nid = $st->fetchColumn();
...
function fin_credit_note_unsync_ledger(PDO $pdo, int $noteId): void
{
    if ($noteId < 1) {
        return;
    }
...
function fin_credit_note_sync_ledger(PDO $pdo, int $noteId): void
{
    ...
    fin_credit_note_unsync_ledger($pdo, $noteId);
    ...
    if ($partyType === 'customer' && crm_ledger_has_table($pdo)) {
        crm_ledger_insert($pdo, $partyId, $txnDate, 'credit_note', $noteId, $refNo, 'credit', 0.0, $total, $memo);
    } elseif ($partyType === 'supplier' && crm_supplier_ledger_has_table($pdo)) {
        crm_supplier_ledger_insert($pdo, $partyId, $txnDate, 'credit_note', $noteId, $refNo, 'credit', $total, 0.0, $memo);
    }

    require_once app_path('includes/acc_gl.php');
    acc_gl_post_credit_note($pdo, $noteId);
}

/**
 * @param list<array<string, mixed>> $lines
 */
function fin_credit_note_save(PDO $pdo, array $header, array $lines): int
{
    $id = (int) ($header['id'] ?? 0);
    ...
    if ($id > 0) {
        $cur = fin_credit_note_fetch($pdo, $id);
        if (!$cur) {
            throw new RuntimeException('الإشعار غير موجود.');
        }
        $pdo->prepare(
            'UPDATE fin_credit_note SET note_date = ?, party_type = ?, party_id = ?, total = ?, reason = ? WHERE id = ?'
        )->execute([
            $noteDate,
            $partyType,
            $partyId,
        ...
        $pdo->prepare('DELETE FROM fin_credit_note_line WHERE note_id = ?')->execute([$id]);
    } else {
        $noteNo = fin_credit_note_next_no($pdo);
        $ins = $pdo->prepare(
            'INSERT INTO fin_credit_note (note_no, note_date, party_type, party_id, total, reason, created_by)
             VALUES (?,?,?,?,?,?,?)'
        );
        $ins->execute([
            $noteNo,
    ...
    $stLine = $pdo->prepare(
        'INSERT INTO fin_credit_note_line (note_id, item_id, description_ar, qty, unit_price, line_total)
         VALUES (?,?,?,?,?,?)'
    );
    foreach ($lines as $ln) {
    ...
    fin_credit_note_sync_ledger($pdo, $id);

    return $id;
}

function fin_credit_note_delete(PDO $pdo, int $id): void
{
    if ($id < 1) {
        throw new RuntimeException('معرّف غير صالح.');
    }
    if (!fin_credit_note_fetch($pdo, $id)) {
        throw new RuntimeException('الإشعار غير موجود.');
    }
    fin_credit_note_unsync_ledger($pdo, $id);
    $pdo->prepare('DELETE FROM fin_credit_note WHERE id = ?')->execute([$id]);
}
