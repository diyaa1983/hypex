...
function pur_invoice_fetch_by_no(PDO $pdo, string $invoiceNo, string $browseFilter = 'all'): ?array
{
    ...
    return doc_no_fetch_exact_or_fragment(
        $pdo,
        $invoiceNo,
        'SELECT id FROM pur_invoice WHERE invoice_no = ? LIMIT 1',
        [trim($invoiceNo)],
        static fn (string $frag) => pur_invoice_search_ids_by_no_fragment($pdo, $frag, $browseFilter),
        static fn (int $id) => pur_invoice_fetch_by_id($pdo, $id, $browseFilter)
    );
}
