 ...
 * }
 */
function hr_employees_by_department_report_build(PDO $pdo, string $status = 'all', int $departmentId = 0): array
{
    ...
    foreach ($raw as $row) {
        ...
        $groups[$deptKey]['employee_count']++;
        $groups[$deptKey]['total_salary'] += $salary;
    }
...
function hr_employees_by_department_report_title(string $status): string
{
    $status = hr_employees_report_normalize_status($status);

    return match ($status) {
        'active' => 'تقرير الموظفين حسب القسم — على رأس العمل',
        'resigned' => 'تقرير الموظفين حسب القسم — المستقيلون',
        default => 'تقرير الموظفين حسب القسم',
    };
}
