...
/**
 * @return array{rows:list<array<string,mixed>>, pager:array<string,mixed>, total:int}
 */
function mobile_receipt_list_page(
    PDO $pdo,
    string $filter = 'all',
    string $search = '',
    int $limit = 120,
    ...
    $list = fin_voucher_list_fetch($pdo, 'receipt', $filter, $search, $limit, $scopedRepId, $offset);
    ...
    return [
        'rows' => $rows,
        'pager' => $list['pager'] ?? [],
        'total' => (int) ($list['filtered_total'] ?? count($rows)),
    ];
}

/** @return list<array<string, mixed>> */
function mobile_receipt_list_rows(
    PDO $pdo,
    string $filter = 'all',
    string $search = '',
    int $limit = 120,
    ...
    return mobile_receipt_list_page($pdo, $filter, $search, $limit, $offset)['rows'];
}
