...
require_once app_path('includes/fin_credit_note_save.php');

if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'save_credit_note') {
    handle_fin_credit_note_save();
}
...
if (!fin_credit_note_ensure_schema($pdo)) {
    echo '<div class="card"><p class="alert alert-error">جداول إشعارات الدائنة غير موجودة. تأكد من تنفيذ <code>database/schema.sql</code> أو الترحيلات ذات الصلة.</p></div>';
    return;
}
...
$exitUrl = nav_exit_url($activeRoute ?? 'credit_notes');
$newUrl = app_url('index.php?r=credit_notes');
$apiNote = app_url('api/fin_credit_note_view.php');
$apiDelete = app_url('api/fin_credit_note_delete.php');
...
$cssDnPath = app_path('assets/css/fin-credit-note.css');
$cssDn = app_url('assets/css/fin-credit-note.css') . (is_file($cssDnPath) ? '?v=' . (string) filemtime($cssDnPath) : '');
...
$jsPath = app_path('assets/js/fin-credit-note.js');
$jsUrl = app_url('assets/js/fin-credit-note.js') . (is_file($jsPath) ? '?v=' . (string) filemtime($jsPath) : '');
?>
...
customer_picker_json_script($customers, 'fin-cn-customers-json');
echo '<script type="application/json" id="fin-cn-suppliers-json">' . crm_suppliers_picker_json($suppliers) . '</script>' . "\n";
?>

<div class="dashboard-ora sales-ora12-screen sales-inv-wrap sales-inv-main fin-cn-wrap sales-inv-bold">
    <header class="dashboard-ora-screen-title no-print" role="banner">
        <h1 class="dashboard-ora-screen-title__text">إشعار دائن</h1>
        <?php nav_render_screen_close($activeRoute ?? 'credit_notes'); ?>
    </header>
    ...
    <form id="fin-cn-form" class="master-page-form" method="post" action="<?= esc(app_url('index.php?r=credit_notes')) ?>" novalidate
          data-app-busy-msg="جاري حفظ إشعار الدائن..."
        ...
        <input type="hidden" name="_action" value="save_credit_note">
        <input type="hidden" name="note_id" id="cn_record_id" value="">
        <input type="hidden" name="party_type" id="cn_party_type" value="customer">
        <input type="hidden" name="party_id" id="cn_party_id" value="">
        <input type="hidden" name="lines_json" id="cn_lines_json" value="[]">
        ...
        <header class="sales-inv-doc-header sales-inv-meta-panel">
            <div class="sales-inv-meta-row">
                <div class="sales-inv-meta-item sales-inv-meta-no">
                    <label for="cn_no">رقم الإشعار</label>
                    <div class="sales-inv-no-nav">
                        <button type="button" class="sales-inv-no-arrow" id="cn_no_prev" title="السابق">‹</button>
                        <input class="input input-compact sales-inv-no-input" type="text" name="note_no_display" id="cn_no" readonly tabindex="-1" placeholder="يُولَّد تلقائياً">
                        <button type="button" class="sales-inv-no-arrow" id="cn_no_next" title="التالي">›</button>
                    </div>
                </div>
                <div class="sales-inv-meta-item sales-inv-meta-date">
                    <label for="cn_date">التاريخ *</label>
                    <input class="input input-compact js-date-dmy" type="text" name="note_date" id="cn_date"
                           value="<?= esc(format_date_dmY($today)) ?>" placeholder="يوم-شهر-سنة" dir="ltr" required>
                </div>
                <div class="sales-inv-meta-item fin-cn-party-type-wrap no-print">
                    <span class="field-label">نوع الطرف *</span>
                    <div class="fin-cn-party-type">
                ...
                <div class="fin-cn-party-customer" id="cn_party_customer_wrap">
                    <?= customer_picker_field([
                        'id' => 'cn_customer',
                        'label' => 'العميل *',
                        'compact' => true,
                        'wrapper_class' => 'sales-inv-meta-item sales-inv-meta-customer',
                        'json_id' => 'fin-cn-customers-json',
                        'manual_bind' => true,
                    ]) ?>
                </div>
                <div class="fin-cn-party-supplier" id="cn_party_supplier_wrap" hidden>
                    <?= supplier_picker_field([
                        'id' => 'cn_supplier',
                        'label' => 'المورد *',
                        'compact' => true,
                        'wrapper_class' => 'sales-inv-meta-item sales-inv-meta-customer',
                        'json_id' => 'fin-cn-suppliers-json',
                        'manual_bind' => true,
                    ]) ?>
                </div>
            </div>
            <div class="sales-inv-meta-row fin-cn-reason-row">
                <label class="field" style="flex:1;margin:0;">
                    <span class="field-label">السبب / ملاحظات</span>
                    <input class="input input-compact" type="text" name="reason" id="cn_reason" maxlength="500" placeholder="اختياري">
                </label>
            </div>
        </header>
            </div>
        ...
        <div class="sales-inv-card">
            <div class="sales-inv-lines-toolbar no-print">
                <button type="button" class="btn btn-secondary btn-sm" id="cn_add_line">+ سطر</button>
            </div>
            <div class="table-wrap sales-inv-table-wrap">
                <table class="data-table sales-inv-lines" id="cn_lines_table">
                    <thead>
                    <tr>
                    ...
                    <tbody id="cn_lines_body"></tbody>
                    <tfoot>
                    <tr>
                        <td colspan="4" class="sales-inv-tfoot-label">الإجمالي</td>
                        <td class="sales-inv-col-total" id="cn_grand_total">0.00</td>
                        <td></td>
                    </tr>
                    </tfoot>
                </table>
...
<template id="fin-cn-line-template">
    <tr class="fin-cn-line" data-line-id="">
