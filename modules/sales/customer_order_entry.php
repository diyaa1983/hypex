...
<?php
/**
 * تصميم شاشة طلب شراء عميل بطراز Oracle (مثل acc-DKzaman).
 * للرجوع للتصميم السابق انسخ:
 *   assets/css/customer-order-doc.css.bak  → customer-order-doc.css
 *   modules/sales/customer_order_entry.php.bak → customer_order_entry.php
 */
?>

<div class="dashboard-ora sales-ora12-screen sales-inv-wrap sales-inv-main co-doc co-doc--dkzaman" data-exit-guard="custom">
    ...
    <div class="dashboard-ora-workspace">
        ...
        <div class="toolbar oracle-toolbar co-ora-toolbar no-print" id="coMainToolbar" role="toolbar" aria-label="أوامر الطلب">
            <a href="<?= esc($exitUrl) ?>" class="btn-toolbar btn-toolbar-exit" title="خروج">خروج</a>
            <div class="toolbar-actions">
                <a class="btn-toolbar" href="<?= esc($newUrl) ?>" title="طلب جديد">جديد</a>
                <button type="button" class="btn-toolbar btn-toolbar-save" id="co-save" title="حفظ">حفظ</button>
                <?php if ($canApprove): ?>
                    <button type="button" class="btn-toolbar btn-toolbar-save" id="co-approve" title="اعتماد">اعتماد</button>
                <?php endif; ?>
                <?php if ($canDelete): ?>
                    <button type="button" class="btn-toolbar btn-danger-tool" id="co-delete" title="حذف" <?= $isNew ? 'disabled' : '' ?>>حذف</button>
                <?php else: ?>
                    <button type="button" class="btn-toolbar btn-danger-tool" id="co-delete" hidden disabled></button>
                <?php endif; ?>
                <span class="co-toolbar-sep" aria-hidden="true"></span>
                <a class="btn-toolbar" href="<?= esc($listUrl) ?>" title="بحث / قائمة الطلبات">بحث</a>
                <?php if (user_can('sales_customer_orders_approve')): ?>
                    <a class="btn-toolbar" href="<?= esc(app_url('index.php?r=sales_customer_orders_approve')) ?>" title="اعتماد الطلبات">اعتمادات</a>
                <?php endif; ?>
            </div>
        </div>

        <form id="customer-order-form" class="master-page-form co-doc-form form-card oracle-rec-screen"
            ...
            <div class="oracle-rec-canvas">

            <div class="oracle-rec-block">
                <div class="oracle-rec-block-caption">بيانات السند</div>
                <div class="oracle-rec-fields">
                    <div class="oracle-rec-field">
                        <label class="oracle-rec-label" for="co_order_no">رقم السند</label>
                        <div class="doc-navigator oracle-doc-nav sales-inv-no-nav">
                            <a class="btn-nav sales-inv-no-arrow" href="<?= esc($listUrl) ?>" title="قائمة الطلبات" aria-label="بحث">‹</a>
                            <input class="oracle-rec-input sales-inv-no-input" type="text" id="co_order_no"
                                   value="<?= esc($orderNo) ?>" placeholder="يُولَّد عند الحفظ" readonly>
                            <a class="btn-nav sales-inv-no-arrow" href="<?= esc($newUrl) ?>" title="طلب جديد" aria-label="جديد">›</a>
                        </div>
                    </div>
                    <div class="oracle-rec-field">
                        <label class="oracle-rec-label" for="order-date">تاريخ السند</label>
                        <input class="oracle-rec-input input js-date-dmy" type="text" id="order-date"
                               value="<?= esc(format_date_dmY($orderDate)) ?>"
                               placeholder="يوم-شهر-سنة" dir="ltr"
                               autocomplete="off" inputmode="numeric" required>
                    </div>
                    <div class="oracle-rec-field oracle-rec-field-wide">
                        <?= customer_picker_field([
                            'id' => 'co_customer',
                            'label' => 'العميل',
                            'compact' => true,
                            'wrapper_class' => 'sales-inv-meta-item sales-inv-meta-customer co-ora-customer',
                    ...
                    <div class="oracle-rec-field">
                        <label class="oracle-rec-label" for="co_payment_type">النوع</label>
                        <select class="oracle-rec-input input" id="co_payment_type" name="payment_type">
                    ...
                    <div class="oracle-rec-field">
                        <label class="oracle-rec-label" for="warehouse-id">المستودع</label>
                        <select class="oracle-rec-input input" id="warehouse-id" required>
            ...
            <div class="oracle-rec-block">
                <div class="oracle-rec-block-caption">تفاصيل المواد</div>
                <div class="oracle-rec-grid-wrap sales-inv-table-wrap" id="sales-inv-table-wrap">
                    <table class="sales-inv-table oracle-rec-grid">
                        <thead>
                ...
                <div class="sales-inv-footer-grid co-doc-footer oracle-rec-fields oracle-rec-fields-second">
                    ...
                    <div class="sales-inv-totals co-doc-totals">
                        <div class="row sales-inv-totals-disc">
                            ...
                            <input type="text" class="oracle-rec-input input input-compact input-num" name="invoice_discount" id="inv-invoice-discount"
                ...
                <div class="co-doc-sumbar oracle-rec-status no-print" aria-live="polite">
                    <div class="co-doc-sumbar__item oracle-status-item">
                    ...
                    <div class="co-doc-sumbar__item co-doc-sumbar__item--grand oracle-status-item">
                        <span>الإجمالي</span>
            ...
            <div class="oracle-rec-block co-ora-ar-panel" id="co-ora-ar-panel" hidden>
                <div class="oracle-rec-block-caption">كشف حساب العميل (Oracle) — المستحقات والشيكات</div>
                <div class="oracle-rec-fields oracle-rec-fields-second" style="display:block">
                    ...
                    <div class="co-ora-ar-summary" id="co-ora-ar-summary" hidden>
                        ...
                        <div class="co-ora-ar-chq-wrap">
                            ...
                            <div class="co-ora-ar-table-wrap oracle-rec-grid-wrap">
                                <table class="co-ora-ar-table oracle-rec-grid">
                                    <thead>
                                    <tr>
                                        <th>رقم الشيك</th>
                        ...
                        <p class="co-ora-ar-actions">
                            <a class="btn-toolbar" id="co-ora-ar-full-link" href="#" target="_blank" rel="noopener">فتح الكشف التفصيلي من Oracle</a>
                            <button type="button" class="btn-toolbar" id="co-ora-ar-refresh">تحديث من Oracle</button>
                        </p>
                    </div>
                </div>
            </div>

            </div><!-- /.oracle-rec-canvas -->
        </form>
    </div>
</div>
