...
if (isset($_GET['run']) && (string) $_GET['run'] === '1') {
    if ($warehouseId < 1) {
        ...
        if (!$whRow) {
            ...
            $showResult = true;
            $rows = inv_report_warehouse_negative_qty_lines($pdo, $warehouseId);
        }
    }
}
...
$reportTitle = 'تقرير المواد السالبة';

$pageDataAttrs = ' data-report-title="' . esc($reportTitle) . '"';
$pageDataAttrs .= ' data-report-route="report_warehouse_negative_qty"';
if ($showResult && $err === '') {
...
<div class="card report-sales-page report-warehouse-negative-qty-page"<?= $pageDataAttrs ?>>
    ...
    <form method="get" action="<?= esc(app_url('index.php')) ?>" class="report-sales-filters no-print">
        <input type="hidden" name="r" value="report_warehouse_negative_qty">
    ...
    <?php if ($showResult && $err === ''): ?>
        <div class="report-sales-result report-sales-print-area report-warehouse-negative-qty-print">
                ...
                <table>
                    ...
                    <tr>
                        <td class="muted" style="font-size:0.9rem;">
                            المواد النشطة التي رصيدها سالب في المستودع (من حركات المخزون المرحّلة).
                        </td>
                    </tr>
                </table>
            </div>
            ...
            <div class="report-sales-table-wrap">
                <table class="report-sales-table report-warehouse-negative-qty-table">
                    <thead>
                    <tr>
                    ...
                    <?php if (!$rows): ?>
                        <tr>
                            <td colspan="4" class="muted" style="text-align:center;padding:1.25rem;">
                                لا توجد مواد برصيد سالب في هذا المستودع.
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php $seq = 0; ?>
                        <?php foreach ($rows as $r): ?>
                            ...
                            <tr>
                                ...
                                <td class="col-qty report-warehouse-negative-qty-cell"><?= esc(format_amount((float) ($r['qty'] ?? 0))) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                    </tbody>
