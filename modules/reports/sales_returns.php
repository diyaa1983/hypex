...
require_once app_path('includes/sal_returns_report.php');
...
if ($submitted) {
    if ($customerId < 0) {
        ...
        if ($fromIso === null || $toIso === null) {
            ...
            if ($showResult) {
                $rows = sal_report_sales_returns($pdo, $customerId, $from, $to);
                foreach ($rows as $r) {
                    $sumInclusive += (float) ($r['return_inclusive'] ?? 0);
                    $sumExclusive += (float) ($r['return_exclusive'] ?? 0);
                }
            }
        }
    }
...
$pageDataAttrs .= ' data-report-route="report_sales_returns"';
if ($showResult) {
...
$customerIdValue = $customerId >= 0 ? (string) $customerId : '';
?>
<link rel="stylesheet" href="<?= esc($cssUrl) ?>">
<link rel="stylesheet" href="<?= esc($invCssUrl) ?>">
<?php require_once app_path('includes/customer_picker.php'); customer_picker_enqueue_assets(); customer_picker_json_script($customers, 'report-sales-returns-customers-json'); ?>
...
<div class="card report-sales-page"<?= $pageDataAttrs ?>>
    ...
    <form method="get" action="<?= esc(app_url('index.php')) ?>" class="report-sales-filters no-print">
        <input type="hidden" name="r" value="report_sales_returns">
        <div class="form-row">
            <?= customer_picker_field([
                'id' => 'report_sales_returns_cust',
                'name' => 'customer_id',
                ...
                'allow_all' => true,
                'json_id' => 'report-sales-returns-customers-json',
            ]) ?>
            <label class="field">
    ...
    <?php if ($showResult): ?>
        <div class="report-sales-result report-sales-print-area">
            ...
            <div class="report-sales-table-wrap">
                <table class="report-sales-table report-sales-table--returns">
                    <colgroup>
                        <col class="col-seq">
                    ...
                    <tr>
                        ...
                        <th class="col-money">قيمة الإرجاع غير شامل</th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php if (!$rows): ?>
                    ...
                    foreach ($rows as $r):
                        ...
                        <tr>
                            ...
                            <td class="col-money"><?= esc(format_money((float) $r['return_exclusive'])) ?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                    <?php if ($rows): ?>
