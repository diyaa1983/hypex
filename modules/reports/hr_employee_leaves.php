...
require_once app_path('includes/hr_employee_leaves_report.php');
...
$departments = hr_employee_leaves_report_department_options($pdo);
$leaveTypes = hr_employee_leaves_report_leave_type_options($pdo);

$report = null;
if ($showReport) {
    $report = hr_employee_leaves_report_build($pdo, $dateFrom, $dateTo, $departmentId, $leaveTypeId);
}
...
$reportTitle = 'تقرير الإجازات بين تاريخين';
$reportDate = date('Y-m-d');
$exitUrl = nav_exit_url('report_hr_employee_leaves');
...
<div class="card report-sales-page hr-ld-rpt-page" data-report-route="report_hr_employee_leaves" data-exit-url="<?= esc($exitUrl) ?>">

    <form method="get" action="<?= esc(app_url('index.php')) ?>" class="report-sales-filters no-print hr-ld-rpt-filters">
        <input type="hidden" name="r" value="report_hr_employee_leaves">
    ...
    <?php if ($showReport && $report !== null): ?>
        <div class="report-sales-result report-sales-print-area hr-ld-rpt-doc">
            ...
            <?php if (($report['departments'] ?? []) === []): ?>
                <p class="hr-ld-rpt-empty muted">لا توجد إجازات مطابقة للفترة والفلاتر المحددة.</p>
            <?php else: ?>
                <div class="hr-ld-rpt-kpis no-print">
                    ...
                    <div class="hr-ld-rpt-kpi">
                        <span class="hr-ld-rpt-kpi__label">إجمالي الأيام</span>
                        <span class="hr-ld-rpt-kpi__value" dir="ltr"><?= esc(number_format((float) ($report['grand_total_days'] ?? 0), 2, '.', '')) ?></span>
                    </div>
                </div>
                        ...
                        <div class="hr-ld-rpt-table-wrap">
                            <table class="hr-ld-rpt-table report-sales-table">
                                ...
                                <tr>
                                    ...
                                    <th class="hr-ld-rpt-col-type">نوع الإجازة</th>
                                    <th class="col-date">تاريخ الإجازة</th>
                                    <th class="col-date">من تاريخ</th>
                                    <th class="col-date">إلى تاريخ</th>
                                    <th class="hr-ld-rpt-col-days">الأيام</th>
                                ...
                                <?php foreach ($deptBlock['rows'] as $row): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td class="col-date" dir="ltr"><?= esc(format_date_dmY((string) ($row['leave_date'] ?? ''))) ?></td>
                                        <td class="col-date" dir="ltr"><?= esc(format_date_dmY((string) ($row['date_from'] ?? ''))) ?></td>
                                        <td class="col-date" dir="ltr"><?= esc(format_date_dmY((string) ($row['date_to'] ?? ''))) ?></td>
                                        <td class="hr-ld-rpt-col-days num" dir="ltr"><?= esc(number_format((float) ($row['days_count'] ?? 0), 2, '.', '')) ?></td>
                                ...
                                <tr class="hr-ld-rpt-row--dept-total">
                                    ...
                                    <td class="hr-ld-rpt-col-days num" dir="ltr"><?= esc(number_format((float) ($deptBlock['total_days'] ?? 0), 2, '.', '')) ?></td>
                                    <td colspan="2"></td>
                                </tr>
                                </tfoot>
                            </table>
                ...
                <footer class="hr-ld-rpt-grand-wrap">
                    <table class="hr-ld-rpt-grand-table">
                        <tbody>
                        <tr>
                            <th class="hr-ld-rpt-grand-label">المجموع الكلي للإجازات</th>
                            <td class="num" dir="ltr"><?= esc(number_format((float) ($report['grand_total_days'] ?? 0), 2, '.', '')) ?> يوم</td>
                        </tr>
                        </tbody>
                    </table>
                </footer>
