...
$routeCode = (string) ($r ?? 'report_invoice_tax');
...
$sumTotal = 0.0;
$sumTax = 0.0;
$totals = ['sale_tax' => 0.0, 'purchase_tax' => 0.0, 'net' => 0.0, 'total_docs' => 0];
$showResult = false;
$err = '';
...
if ($reportTitle === '') {
    $reportTitle = $kind === 'purchase'
        ? 'الضريبة المستحقة على فواتير الشراء'
        : 'الضريبة المستحقة على فواتير البيع';
}

$pageDataAttrs = ' data-report-title="' . esc($reportTitle) . '"';
$pageDataAttrs .= ' data-report-route="' . esc($routeCode) . '"';
if ($showResult && $err === '') {
    $pageDataAttrs .= ' data-export-label="vat-invoice-tax"';
...
<div class="card report-sales-page"<?= $pageDataAttrs ?>>
    ...
    <form method="get" action="<?= esc(app_url('index.php')) ?>" class="report-sales-filters no-print">
        ...
        <div class="form-row">
            <label class="field">
                ...
                <input class="input js-date-dmy" type="text" name="from" value="<?= esc(format_date_dmY($from)) ?>"
                       placeholder="يوم-شهر-سنة" dir="ltr" autocomplete="off" required>
            </label>
            <label class="field">
        ...
        <p class="muted" style="margin:0 0 0.5rem;font-size:0.85rem;">
            فواتير <?= $kind === 'sale' ? 'بيع' : 'شراء' ?> مرحّلة فقط — <strong>بدون</strong> مردودات.
        </p>
        <div style="margin-top:0.5rem;display:flex;flex-wrap:wrap;gap:0.5rem;">
            <button class="btn btn-primary" type="submit">عرض التقرير</button>
            <?php if ($kind === 'sale'): ?>
                <a class="btn btn-secondary" href="<?= esc(app_url('index.php?r=report_invoice_tax_purchase')) ?>">فواتير الشراء</a>
            <?php else: ?>
                ...
                <table class="report-sales-table">
                    ...
                    <?php else: ?>
                        ...
                        <?php foreach ($rows as $r): ?>
                            ...
                            <tr>
                                ...
                                <td class="col-inv-no"><code><?= esc((string) ($r['doc_no'] ?? '')) ?></code></td>
                                <td class="col-date"><?= esc(format_date_dmY((string) ($r['doc_date'] ?? ''))) ?></td>
                    ...
                    <?php if ($rows): ?>
                        ...
                        <tr class="report-sales-tfoot">
                            <td colspan="4"><strong><?= $kind === 'sale' ? 'مجموع ضريبة البيع' : 'مجموع ضريبة الشراء' ?></strong></td>
