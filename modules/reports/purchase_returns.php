...
require_once app_path('includes/pur_returns_report.php');
...
if ($submitted) {
    if ($supplierId < 0) {
        ...
        if ($fromIso === null || $toIso === null) {
            ...
            if ($showResult) {
                $rows = pur_report_purchase_returns($pdo, $supplierId, $from, $to);
                foreach ($rows as $r) {
...
$pageDataAttrs .= ' data-report-route="report_purchase_returns"';
if ($showResult) {
...
<?php supplier_picker_json_script($suppliers, 'report-purchase-returns-suppliers-json'); ?>
...
<div class="card report-sales-page"<?= $pageDataAttrs ?>>
    ...
    <form method="get" action="<?= esc(app_url('index.php')) ?>" class="report-sales-filters no-print">
        <input type="hidden" name="r" value="report_purchase_returns">
        <div class="form-row">
            <?= supplier_picker_field([
                'id' => 'report_purchase_returns_supp',
                'name' => 'supplier_id',
                ...
                'allow_all' => true,
                'json_id' => 'report-purchase-returns-suppliers-json',
            ]) ?>
            <label class="field">
            ...
            <div class="report-sales-table-wrap">
                <table class="report-sales-table report-sales-table--returns">
                    ...
                    <tr>
                        ...
                        <th class="col-inv-no">رقم فاتورة الشراء المرجع منها</th>
                    ...
                    <?php if (!$rows): ?>
                        <tr>
                            <td colspan="6" class="muted" style="text-align:center;padding:1.25rem;">
                                لا توجد مرتجعات مشتريات مطابقة في الفترة المحددة.
                            </td>
                        </tr>
                    <?php endif; ?>
                    <?php
