...
require_once app_path('includes/pur_purchases_by_item_report.php');
        ...
        if ($fromIso === null || $toIso === null) {
            ...
            if ($supplierId === 0) {
                ...
                if (!$sup) {
                    ...
                    $supplierName = (string) ($sup['name_ar'] ?? '');
                }
            }
            ...
            if ($err === '') {
                $showResult = true;
                $rows = pur_report_purchases_by_item($pdo, $supplierId, $itemId, $from, $to);
            }
        }
    }
} elseif ($itemId === 0) {
...
$reportTitle = 'تقرير المشتريات حسب المادة';

$pageDataAttrs = ' data-report-title="' . esc($reportTitle) . '"';
$pageDataAttrs .= ' data-report-route="report_purchases_by_item"';
if ($showResult) {
    $pageDataAttrs .= ' data-export-label="' . esc($supplierName . ' — ' . $itemLabel) . '"';
...
supplier_picker_json_script($suppliers, 'report-purchases-item-suppliers-json');
?>
...
<div class="card report-sales-page"<?= $pageDataAttrs ?>>
    ...
    <form method="get" action="<?= esc(app_url('index.php')) ?>" class="report-sales-filters no-print">
        <input type="hidden" name="r" value="report_purchases_by_item">
        <div class="form-row">
            <?= supplier_picker_field([
                'id' => 'report_purchases_item_supp',
                'name' => 'supplier_id',
                ...
                'allow_all' => true,
                'json_id' => 'report-purchases-item-suppliers-json',
            ]) ?>
            <?= item_picker_single_field([
                'id' => 'report_purchases_item_id',
                'name' => 'item_id',
    ...
    <?php if ($showResult): ?>
        <div class="report-sales-result report-sales-print-area">
            ...
            <div class="doc-print-meta">
                <table>
                    <tr>
                        <td><strong>المورد:</strong> <span class="doc-print-meta-value doc-print-meta-value--party"><?= esc($supplierName) ?></span></td>
                    </tr>
                    <tr>
                    ...
                    <?php if (!$rows): ?>
                        <tr>
                            <td colspan="6" class="muted" style="text-align:center;padding:1.25rem;">
                                لا توجد بنود مشتريات مطابقة في الفترة المحددة.
                            </td>
                        </tr>
                    <?php endif; ?>
                    <?php
