...
if ($submitted) {
    if ($itemId < 1) {
        ...
        if ($fromIso === null || $toIso === null) {
            ...
            $from = $fromIso;
            $to = $toIso;
            $st = $pdo->prepare('SELECT name_ar, barcode FROM inv_item WHERE id = ? LIMIT 1');
            $st->execute([$itemId]);
            ...
            if (!$it) {
                ...
                $nm = (string) ($it['name_ar'] ?? '');
                $itemDisplayName = $bc !== '' ? ($bc . ' — ' . $nm) : $nm;
                $showResult = true;
                $rows = sal_report_customer_orders_by_item($pdo, $itemId, $from, $to);
                foreach ($rows as $r) {
                    $sumQty += (float) ($r['qty_ordered'] ?? 0);
...
$reportTitle = 'تقرير طلبات الشراء للعميل حسب المادة';
...
$pageDataAttrs .= ' data-report-route="report_customer_orders_by_item"';
if ($showResult) {
...
<div class="card report-sales-page"<?= $pageDataAttrs ?>>
    ...
    <form method="get" action="<?= esc(app_url('index.php')) ?>" class="report-sales-filters no-print">
        <input type="hidden" name="r" value="report_customer_orders_by_item">
        <div class="form-row">
            <?= item_picker_single_field([
                'id' => 'report_co_item_id',
                'name' => 'item_id',
        ...
        <div class="report-sales-result report-sales-print-area">
            ...
            <div class="doc-print-meta">
                <table>
                    ...
                    <tr>
                        <td>
                            <strong>عدد الأسطر:</strong> <?= count($rows) ?>
                            &nbsp;&nbsp;|&nbsp;&nbsp;
                            <strong>إجمالي الكمية:</strong> <?= esc(format_money($sumQty)) ?>
                        </td>
                    </tr>
                </table>
            </div>
            ...
            <div class="report-sales-table-wrap">
                <table class="report-sales-table js-sortable-report"
                    ...
                    <tr>
                        ...
                        <th class="col-customer js-sort-th" data-sort="sales_rep_name" data-sort-type="text">المندوب</th>
                        <th class="col-money js-sort-th" data-sort="qty_ordered" data-sort-type="number">الكمية</th>
                        <th class="col-money js-sort-th" data-sort="unit_name" data-sort-type="text">الوحدة</th>
                    ...
                    foreach ($rows as $r):
                        ...
                        <tr data-sort-row="1"
                            ...
                            <td class="col-customer"><?= esc((string) ($r['sales_rep_name'] ?: '—')) ?></td>
                            <td class="col-money"><?= esc(format_money((float) $r['qty_ordered'])) ?></td>
                            <td class="col-money"><?= esc((string) ($r['unit_name'] ?: '—')) ?></td>
                    ...
                    <tr>
                        ...
                        <td class="col-money"><?= esc(format_money($sumQty)) ?></td>
                        <td></td>
                        <td></td>
