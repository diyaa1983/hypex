...
require_once app_path('includes/hr_employees_by_department_report.php');
...
if ($run) {
    $showResult = true;
    $report = hr_employees_by_department_report_build($pdo, $status, $departmentId);
}
...
$cssDeptPath = app_path('assets/css/hr-employees-by-dept-report.css');
$cssDeptUrl = app_url('assets/css/hr-employees-by-dept-report.css')
    . (is_file($cssDeptPath) ? '?v=' . (string) filemtime($cssDeptPath) : '');
$exportJsPath = app_path('assets/js/report-sales-export.js');
$exportJsUrl = app_url('assets/js/report-sales-export.js') . (is_file($exportJsPath) ? '?v=' . (string) filemtime($exportJsPath) : '');

$reportTitle = hr_employees_by_department_report_title($status);

$pageDataAttrs = ' data-report-title="' . esc($reportTitle) . '"';
$pageDataAttrs .= ' data-report-route="report_hr_employees_by_department"';
if ($showResult) {
    $pageDataAttrs .= ' data-export-label="' . esc(hr_employees_report_status_label($status)) . '"';
}
?>
<link rel="stylesheet" href="<?= esc($cssUrl) ?>">
<link rel="stylesheet" href="<?= esc($cssDeptUrl) ?>">

<div class="card report-sales-page hr-emp-dept-rpt-page"<?= $pageDataAttrs ?>>

    <form method="get" action="<?= esc(app_url('index.php')) ?>" class="report-sales-filters no-print">
        <input type="hidden" name="r" value="report_hr_employees_by_department">
    ...
    <?php if ($showResult && $report !== null): ?>
        <div class="report-sales-result report-sales-print-area hr-emp-dept-rpt-doc">
            ...
            <?php if (($report['departments'] ?? []) === []): ?>
                <p class="hr-emp-dept-rpt-empty muted">لا يوجد موظفون مطابقون للفلتر.</p>
            <?php endif; ?>

            <?php foreach ($report['departments'] as $block): ?>
                <section class="hr-emp-dept-rpt-block">
                    <h3 class="hr-emp-dept-rpt-dept-name">
                        القسم: <?= esc((string) ($block['dept_name'] ?? '')) ?>
                        <span class="hr-emp-dept-rpt-dept-meta">
                            (<?= (int) ($block['employee_count'] ?? 0) ?> موظف)
                        </span>
                    </h3>
                    <div class="report-sales-table-wrap">
                        <table class="report-sales-table hr-emp-dept-rpt-table">
                            <thead>
                            <tr>
                                <th class="col-seq">تسلسل</th>
                                <th class="col-inv-no">رقم الموظف</th>
                                <th class="col-customer-name">اسم الموظف</th>
                                <th class="hr-emp-dept-rpt-col-job">المسمى الوظيفي</th>
                            ...
                            <?php foreach ($block['rows'] as $row): ?>
                                <tr>
                                    ...
                                    <td class="hr-emp-dept-rpt-col-job"><?= esc((string) ($row['job_title_name'] ?? '—')) ?></td>
                            ...
                            <tfoot>
                            <tr class="hr-emp-dept-rpt-sum">
                                <td colspan="5"><strong>مجموع القسم</strong></td>
            ...
            <?php if ($report['departments']): ?>
                <footer class="hr-emp-dept-rpt-grand">
                    <table class="hr-emp-dept-rpt-grand-table">
                        <tbody>
                        <tr>
