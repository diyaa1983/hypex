...
require_once app_path('includes/crm_suppliers_report.php');
...
if ($run) {
    $showResult = true;
    $rows = crm_report_suppliers_list($pdo, $activeOnly);
}
...
$pageDataAttrs .= ' data-report-route="report_suppliers"';
if ($showResult) {
    $pageDataAttrs .= ' data-export-label="suppliers"';
}
?>
<link rel="stylesheet" href="<?= esc($cssUrl) ?>">

<div class="card report-sales-page report-suppliers-page"<?= $pageDataAttrs ?>>

    <form method="get" action="<?= esc(app_url('index.php')) ?>" class="report-sales-filters no-print">
        <input type="hidden" name="r" value="report_suppliers">
        <input type="hidden" name="run" value="1">
        <div class="form-row" style="align-items:center;">
            <label class="field" style="flex:0 0 auto;">
                <input type="checkbox" name="active_only" value="1" <?= $activeOnly ? 'checked' : '' ?>>
                <span style="margin-right:0.35rem;">الموردون النشطون فقط</span>
            </label>
        </div>
    ...
    <?php if ($showResult): ?>
        <div class="report-sales-result report-sales-print-area report-suppliers-print">
            ...
            <div class="doc-print-meta">
                <table>
                    <tr>
                        <td>
                            <strong>عدد الموردين:</strong> <?= count($rows) ?>
                            &nbsp;&nbsp;|&nbsp;&nbsp;
                    ...
                    <?php if ($activeOnly): ?>
                        <tr>
                            <td class="muted" style="font-size:0.9rem;">يعرض الموردين النشطين فقط.</td>
                        </tr>
                    <?php endif; ?>
                </table>
            </div>
            ...
            <div class="report-sales-table-wrap">
                <table class="report-sales-table report-suppliers-table">
                    <thead>
                    <tr>
                        <th class="col-seq">#</th>
                        <th class="col-inv-no">رمز المورد</th>
                        <th class="col-customer-name">اسم المورد</th>
                    ...
                    <?php else: ?>
                        ...
                        <?php foreach ($rows as $r): ?>
                            ...
                            <tr>
                                ...
                                <td class="col-inv-no"><code><?= esc((string) ($r['supplier_code'] ?? '')) ?></code></td>
                                <td class="col-customer-name"><?= esc((string) ($r['supplier_name'] ?? '')) ?></td>
