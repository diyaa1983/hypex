...
$listUrl = app_url('index.php?r=purchase_documents_list');
$newUrl = app_url('index.php?r=purchase_invoices');
$exitUrl = nav_exit_url('purchase_documents_list');

$pager = list_pager_from_request($pdo);
$result = pur_invoices_list_fetch($pdo, $search, $pager);
$rows = $result['rows'];
$pager = $result['pager'];
...
$listPagerUrl = list_pager_base_url('purchase_documents_list', $listPagerQuery);

$flash = flash_get();
?>
<?php sales_ora12_enqueue_assets(); ?>

<div class="dashboard-ora sales-ora12-screen sales-ora-list-page sales-inv-list-page sal-documents-list-page pur-invoices-list-page"
     data-exit-url="<?= esc($exitUrl) ?>">
    <?php sales_ora12_render_title_bar('قائمة فواتير الشراء'); ?>
    ...
    <div class="sales-ora-panel card">
        <div class="table-wrap">
            <table class="data-table" id="pur-invoices-table">
                <thead>
                <tr>
                ...
                foreach ($rows as $row):
                    ...
                    $openUrl = pur_documents_list_invoice_open_url($docId);
                    ?>
                    <tr class="sal-doc-list-row is-clickable"
                        ...
                        <td>
                            ...
                            <?php if ($returnCount > 0): ?>
                                <span class="badge badge-warn sal-inv-has-return" style="margin-inline-start:0.35rem;">مردودة</span>
                                <?php if ($returnNos !== ''): ?>
                                    <div class="muted sal-return-nos-hint" style="font-size:0.78rem;margin-top:0.2rem;">
                                        مردود: <code><?= esc($returnNos) ?></code>
                                    </div>
                                <?php endif; ?>
                            <?php endif; ?>
                        </td>
