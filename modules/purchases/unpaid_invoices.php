...
$listUrl = app_url('index.php?r=purchase_unpaid_invoices');
$exitUrl = nav_exit_url('purchase_unpaid_invoices');

$allRows = pur_unpaid_invoices_list($pdo);
if ($search !== '') {
...
$listPagerUrl = list_pager_base_url('purchase_unpaid_invoices', $listPagerQuery);
...
<div class="dashboard-ora sales-ora12-screen sales-ora-list-page sales-inv-list-page"
    ...
    <?php sales_ora12_render_title_bar('فواتير الشراء غير المدفوعة'); ?>
    ...
    <div class="sales-ora-panel card">
        <form method="get" action="<?= esc(app_url('index.php')) ?>" class="sales-ora-search-form form-row">
            <input type="hidden" name="r" value="purchase_unpaid_invoices">
        ...
        <p class="muted" style="margin:0.65rem 0 0;font-size:0.85rem;">
            <?= (int) $listTotal ?> فاتورة غير مدفوعة
            · إجمالي المتبقي:
                ...
                <?php if ($rows === []): ?>
                    <tr>
                        <td colspan="6" class="muted" style="text-align:center;padding:1.25rem;">
                            لا توجد فواتير شراء غير مدفوعة.
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($rows as $i => $row): ?>
                        ...
                        $supLabel = trim((string) ($row['supplier_code'] ?? ''));
                        $supLabel = $supLabel !== ''
                            ? $supLabel . ' — ' . (string) ($row['supplier_name'] ?? '')
                            : (string) ($row['supplier_name'] ?? '');
                        ?>
                        <tr>
