...
if (!is_logged_in() || !mobile_can_access_receipt_api() || !user_can_action('action_edit_cash_receipt')) {
...
if ($id < 1) {
    echo json_encode(['ok' => false, 'message' => 'حدد سند القبض أولاً.'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
if (!$row) {
    echo json_encode(['ok' => false, 'message' => 'سند القبض غير موجود.'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
try {
    ...
    $result = fin_voucher_unpost_receipts_by_ids($pdo, [$id]);
