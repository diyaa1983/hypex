...
if (!is_logged_in() || !user_can('debit_notes')) {
...
if ($edge === 'first') {
    $st = $pdo->query('SELECT id FROM fin_debit_note ORDER BY id DESC LIMIT 1');
    $fid = $st->fetchColumn();
    if ($fid === false) {
        echo json_encode(['ok' => false, 'error' => 'empty', 'message' => 'لا توجد إشعارات مدينة.'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    $id = (int) $fid;
}
...
if ($id > 0 && $dir === 'prev') {
    $nid = fin_debit_note_nav_neighbor_id($pdo, $id, 'prev');
    $id = $nid ?? 0;
} elseif ($id > 0 && $dir === 'next') {
    $nid = fin_debit_note_nav_neighbor_id($pdo, $id, 'next');
    $id = $nid ?? 0;
}

$row = null;
if ($id > 0) {
    $row = fin_debit_note_fetch($pdo, $id);
} elseif ($no !== '') {
    $st = $pdo->prepare('SELECT id FROM fin_debit_note WHERE note_no = ? LIMIT 1');
    $st->execute([$no]);
    $fid = $st->fetchColumn();
    if ($fid !== false) {
        $row = fin_debit_note_fetch($pdo, (int) $fid);
    }
}
...
$lines = fin_debit_note_fetch_lines($pdo, $noteId);
$outLines = [];
foreach ($lines as $ln) {
    $outLines[] = [
        'id' => (int) $ln['id'],
...
echo json_encode([
    ...
    'note' => [
        ...
        'prev_id' => fin_debit_note_nav_neighbor_id($pdo, $noteId, 'prev') ?? 0,
        'next_id' => fin_debit_note_nav_neighbor_id($pdo, $noteId, 'next') ?? 0,
        'lines' => $outLines,
    ],
