 ...
 *
 * GET:
 *   online_seconds  (افتراضي 120) — يعتبر «متصل» خلال هذه الثواني
 *   stale_seconds   (افتراضي 120) — يظهر فقط من حدّث موقعه خلال هذه المدة
...
try {
    ...
    $onlineSeconds = isset($_GET['online_seconds'])
        ...
        : (isset($_GET['online_minutes']) ? (int) $_GET['online_minutes'] * 60 : 120);
    $staleSeconds = isset($_GET['stale_seconds'])
        ? (int) $_GET['stale_seconds']
        : (isset($_GET['stale_minutes']) ? (int) $_GET['stale_minutes'] * 60 : 120);
