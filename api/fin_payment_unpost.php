...
if (!is_logged_in() || !user_can('cash_payment') || !user_can_action('action_unpost_cash_payment')) {
...
try {
    ...
    $result = fin_voucher_unpost_payments_by_ids($pdo, $ids);
    ...
    if ($unposted === 0 && $skipped > 0) {
        ...
        $msg = 'تم إلغاء ترحيل ' . $unposted . ' سند صرف (كشف المورد/العميل + القيد المحاسبي).';
        if ($skipped > 0) {
