...
if (in_array($status, ['closed', 'cancelled'], true)) {
    echo json_encode(['ok' => false, 'message' => 'لا يمكن إلغاء طلب مغلق أو ملغى.'], JSON_UNESCAPED_UNICODE);
    exit;
}

$pdo->prepare("UPDATE pur_order SET status = 'cancelled' WHERE id = ?")->execute([$orderId]);

echo json_encode(['ok' => true, 'message' => 'تم إلغاء طلب الشراء.', 'order_id' => $orderId], JSON_UNESCAPED_UNICODE);
