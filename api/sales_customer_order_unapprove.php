...
if (!is_logged_in() || !sal_customer_order_user_can_unapprove() || $_SERVER['REQUEST_METHOD'] !== 'POST') {
    http_response_code(403);
    echo json_encode(['ok' => false, 'error' => 'forbidden', 'message' => 'لا توجد صلاحية فك اعتماد الطلب.'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
try {
    ...
    sal_customer_order_set_approved($pdo, (int) ($data['id'] ?? 0), false, (int) (current_user()['id'] ?? 0));
    require_once app_path('includes/header_check_notifications.php');
    header_check_notifications_invalidate_cache();
    echo json_encode(['ok' => true, 'message' => 'تم فك الاعتماد.'], JSON_UNESCAPED_UNICODE);
} catch (Throwable $e) {
