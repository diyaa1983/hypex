-- ضمان اجتماعي: حساب مستحق واحد فقط في شاشة الربط (بدون حصة موظف/شركة منفصلة)

UPDATE acc_posting_setting SET
    label_ar = 'ضمان اجتماعي مستحق',
    hint_ar = 'دائن عند ترحيل الرواتب — مجموع حصة الموظف + حصة الشركة للتسديد للضمان الاجتماعي'
...
SELECT '2006', 'ضمان اجتماعي مستحق', p.id, 'liability', 1, 1, 28
FROM acc_account p
...
INNER JOIN acc_account a ON a.is_active = 1 AND a.is_leaf = 1
    AND (a.code = '2006' OR a.name_ar LIKE '%ضمان%مستحق%')
