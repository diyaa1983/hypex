(function () {
  ...
  if (!tbody || !tpl) {
    console.error('purchase-invoice: missing table body or line template');
    return;
  }
  ...
  var draftKey = form.getAttribute('data-draft-key') || 'purchase_invoices';
  ...
  function safeEnsureEntryRow() {
    try {
      ...
      console.error('purchase-invoice: ensureEntryRow failed', e);
      return null;
    }
  }
  ...
  function blockItemPickUntilQty(tr, opts) {
    opts = opts || {};
    if (invoiceIsPosted || invoiceIsCancelled || ledgerView) return false;
  ...
  function submitInvoiceForm() {
    ...
    fetch(actionUrl, {
        ...
        if (ct.indexOf('application/json') >= 0) {
          return res.json().then(function (data) {
            if (!data || !data.ok) {
              formSubmitting = false;
              setSaveBusy(false);
              var msg = (data && data.message) || 'تعذر حفظ الفاتورة.';
      ...
      .catch(function (err) {
        console.error('purchase-invoice save', err);
        formSubmitting = false;
        setSaveBusy(false);
        if (global.AppDialog) {
  ...
  function openPickerForRow(tr, opts) {
    opts = opts || {};
    if (invoiceIsPosted || invoiceIsCancelled) return;
  ...
  function buildInvoicePrintInnerHtml() {
    ...
    if (supInvNo) {
      metaRows.splice(1, 0, { label: 'رقم فاتورة المورد', value: supInvNo });
    }
    var metaTable =
    ...
    var inner =
      buildDocPrintHeader('فاتورة شراء') +
      metaTable +
  ...
  function buildStandaloneInvoiceHtml() {
    var bodyAttrs =
      window.DocumentHeader && window.DocumentHeader.bodyPrintAttrs
        ...
        : '';
    return (
      '<!DOCTYPE html><html ' + (typeof appPrintHtmlLangAttrs === 'function' ? appPrintHtmlLangAttrs() : 'lang="ar" dir="rtl"') + '><head><meta charset="utf-8"><title>' + (typeof __ === 'function' ? __('فاتورة شراء') : 'فاتورة شراء') + '</title>' +
      '<style>' +
      getPrintFrameStyles() +
      '</style></head><body' +
      bodyAttrs +
  ...
  function updateHistory(id) {
    ...
    var base = newInvoiceUrl || window.location.pathname + '?r=purchase_invoices';
  ...
  function postCurrentInvoice() {
    ...
    AppDialog.confirm(
      'ترحيل هذه الفاتورة (إدخال مخزون + ذمة المورد)؟',
      { title: 'ترحيل' }
    ).then(function (ok) {
      if (!ok) return;
      var fd = new FormData();
      ...
      fetch(invoicePostUrl, { method: 'POST', body: fd, credentials: 'same-origin' })
        ...
        .then(function (res) {
          ...
          updatePostedBadge();
          var successMsg = data.message || 'تم الترحيل.';
          if (data.warning) {
            successMsg += '\n\nتنبيه: ' + data.warning;
          }
  ...
  function unpostCurrentInvoice() {
    ...
    var csrfInput = form.querySelector('[name="_csrf"]');
    var msg =
      'سيتم فك ترحيل الفاتورة:\n' +
      '• إلغاء القيد المحاسبي تلقائياً (يَختفي من تقارير الأستاذ وميزان المراجعة).\n' +
      '• إعادة الأرصدة المخزنية للمواد (إلغاء حركات الإدخال).\n' +
      '• إلغاء أثر حساب المورد (الذمم الدائنة).\n\n' +
      'بعد فك الترحيل يمكنك تعديل الفاتورة وإعادة ترحيلها.\n\nمتابعة؟';
