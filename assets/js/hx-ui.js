...
(function () {
        ...
        kind === 'error'
          ...
          : kind === 'ok' || kind === 'success'
            ...
            : kind === 'warn' || kind === 'warning'
              ...
              : 'info';
      var title =
        type === 'error' ? 'تعذّر الإكمال' : type === 'success' ? 'تم' : type === 'warning' ? 'تحذير' : 'تنبيه';
      return window.AppDialog.alert(String(message || ''), {
        type: type,
        title: title,
        theme: 'oracle',
      });
  ...
  function confirm(message, opts) {
    ...
    if (window.AppDialog && typeof window.AppDialog.confirm === 'function') {
      return window.AppDialog.confirm(String(message || ''), {
        title: opts.title || 'تأكيد',
        okText: opts.okLabel || 'موافق',
        cancelText: opts.cancelLabel || 'إلغاء',
        danger: !!opts.danger,
        type: opts.kind === 'error' ? 'error' : opts.kind === 'warning' || opts.kind === 'warn' ? 'warning' : 'confirm',
        theme: 'oracle',
      });
    }
    return showDialog({
