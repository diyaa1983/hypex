(function () {
  ...
  var screen = document.getElementById('fin-receipts-list-screen');
  if (!screen) return;
  ...
  var unpostedEl = document.getElementById('fin-rc-unposted-count');
  var postSelectedBtn = document.getElementById('fin-rc-post-selected');
  var checkAll = document.getElementById('fin-rc-check-all');
  ...
  function postVouchers(ids, onDone) {
    ...
    fetch(postUrl, { method: 'POST', body: fd, credentials: 'same-origin' })
      ...
      .then(function (data) {
        ...
        if (data.remaining_receipts !== undefined) {
          updateUnpostedCount(data.remaining_receipts);
        }
  ...
  function selectedIds() {
    var ids = [];
    document.querySelectorAll('.fin-rc-row-check:checked').forEach(function (cb) {
  ...
  document.querySelectorAll('.fin-rc-row-check').forEach(function (cb) {
  ...
  if (postSelectedBtn) {
    postSelectedBtn.addEventListener('click', function () {
      ...
      AppDialog.confirm('ترحيل ' + ids.length + ' سند قبض محدد؟', { title: 'ترحيل' }).then(function (ok) {
        if (ok) postVouchers(ids);
      });
    });
  }

  document.querySelectorAll('.fin-rc-post-one').forEach(function (btn) {
    btn.addEventListener('click', function () {
      var id = parseInt(btn.getAttribute('data-id'), 10);
      if (id < 1) return;
      AppDialog.confirm('ترحيل سند الصرف (قيد دائن على حساب العميل (دفعة)؟', { title: 'ترحيل' }).then(function (ok) {
  ...
  document.querySelectorAll('.fin-rc-delete-one').forEach(function (btn) {
    btn.addEventListener('click', function () {
      var id = parseInt(btn.getAttribute('data-id'), 10);
  ...
  document.addEventListener('master-toolbar', function (e) {
    ...
    if (e.detail.action === 'post') {
      ...
      var ids = selectedIds();
      if (!ids.length) {
        AppDialog.alert('حدّد سند قبض واحد على الأقل غير مرحّل.', { type: 'warning' });
        return;
      }
      AppDialog.confirm('ترحيل ' + ids.length + ' سند قبض (قيد دائن على حساب العميل (دفعة)؟', {
        title: 'ترحيل',
      }).then(function (ok) {
        if (ok) postVouchers(ids);
      });
    ...
    if (e.detail.action === 'delete') {
      ...
      if (!delIds.length) {
        AppDialog.alert('حدّد سند قبض واحد على الأقل غير مرحّل للحذف.', { type: 'warning' });
        return;
      }
      var labels = [];
      document.querySelectorAll('.fin-rc-row-check:checked').forEach(function (cb) {
