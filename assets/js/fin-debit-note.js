(function () {
  ...
  var form = document.getElementById('fin-dn-form');
  if (!form) return;
  ...
  var recordIdInp = document.getElementById('dn_record_id');
  var partyTypeInp = document.getElementById('dn_party_type');
  var partyIdInp = document.getElementById('dn_party_id');
  var linesJsonInp = document.getElementById('dn_lines_json');
  var tbody = document.getElementById('dn_lines_body');
  var tpl = document.getElementById('fin-dn-line-template');
  var grandTotalEl = document.getElementById('dn_grand_total');
  var noInp = document.getElementById('dn_no');
  var dateInp = document.getElementById('dn_date');
  var reasonInp = document.getElementById('dn_reason');
  var custWrap = document.getElementById('dn_party_customer_wrap');
  var suppWrap = document.getElementById('dn_party_supplier_wrap');
  ...
  function getPartyId() {
    var pt = getPartyType();
    var h = document.getElementById(pt === 'supplier' ? 'dn_supplier' : 'dn_customer');
  ...
  function renumberLines() {
    if (!tbody) return;
    var i = 0;
    tbody.querySelectorAll('tr.fin-dn-line').forEach(function (tr) {
      i++;
  ...
  function bindLine(tr) {
    ...
    if (rm) {
      rm.addEventListener('click', function () {
        ...
        renumberLines();
        calcGrandTotal();
        if (!tbody.querySelector('tr.fin-dn-line')) addLine();
      });
    }
  ...
  function setBrowseNav(prevId, nextId) {
    ...
    if (window.DocumentNoNav) {
      DocumentNoNav.updateButtons('dn_no_prev', 'dn_no_next', browseNavPrevId, browseNavNextId, {
        onEmpty: currentId < 1,
        prevTitle: 'الإشعار السابق',
        nextTitle: 'الإشعار التالي',
        prevBeforeLatestTitle: 'الإشعار قبل الأخير',
        latestTitle: 'آخر إشعار مدين',
      });
    }
  }
  ...
  function applyNote(note) {
    ...
    if (pt === 'customer' && customerPickerApi) {
      ...
      var disp = document.getElementById('dn_customer_display');
      ...
      var sdisp = document.getElementById('dn_supplier_display');
    ...
    if (!tbody || !tbody.querySelector('tr.fin-dn-line')) addLine();
    calcGrandTotal();
    ...
    fetch(url, { credentials: 'same-origin' })
      ...
      .then(function (data) {
        ...
        if (window.history && window.history.replaceState) {
          ...
          u.searchParams.set('r', 'debit_notes');
  ...
  function deleteNote() {
    ...
    if (!apiDelete) return;
    if (!window.confirm('حذف إشعار المدينة الحالي؟')) return;
    var fd = new FormData();
  ...
  var addBtn = document.getElementById('dn_add_line');
  if (addBtn) addBtn.addEventListener('click', function () {
    addLine();
  });

  var prevBtn = document.getElementById('dn_no_prev');
  var nextBtn = document.getElementById('dn_no_next');
  if (prevBtn) {
  ...
  if (window.CustomerPickerModal) {
    customerPickerApi = CustomerPickerModal.bind({
      hidden: 'dn_customer',
      open: 'dn_customer_open',
      display: 'dn_customer_display',
      jsonId: 'fin-dn-customers-json',
      onSelect: function () {
  ...
  if (window.SupplierPickerModal) {
    supplierPickerApi = SupplierPickerModal.bind({
      hidden: 'dn_supplier',
      open: 'dn_supplier_open',
      display: 'dn_supplier_display',
      jsonId: 'fin-dn-suppliers-json',
      onSelect: function () {
